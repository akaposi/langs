...
Norm = record
      { 
          ...
          ι∙ = (λ _ → ⊥ , λ {()}), λ {()};  
          _⇒∙_ = λ {A}{B} A∙ B∙ → (λ t → (({u : I.Tm A} → fst (fst A∙ u) → fst (fst B∙ (t I.· u))) × N.Nf (A I.⇒ B) t ) , isSet× (isSetImplicitΠ λ u → isSet→ (snd ((fst B∙) (t I.· u)))) (Discrete→isSet λ {x y → {!  N._≟_ !}})) , snd ; --discreteNf x y refl refl
