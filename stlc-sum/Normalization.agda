...
open import Cubical.Data.Equality renaming (_≡_ to _Ind≡_; transport to indtransport; refl to indrefl; _∙_ to _Ind∙_; sym to Indsym; J to IndJ) hiding (assoc; id; step-≡;  _∎; Iso;  funExt)
module stlc-sum.Normalization where
...
module Normalization where
  ...
  -- +-η : (t : Tm Γ (A +ₗ B) tˢ)(a : Tm (Γ ▸ A) A aˢ)(b : Tm (Γ ▸ B) B bˢ) → case+ t (inlₗ a) (inrₗ b)  ≡ᵗ[ S.+-η tˢ aˢ bˢ ] t 
  -- +-η {Γ = Γ} t a b = Tm-path {!   !}
  -- (λ γ i → +-η-helper (∣ t ∣ γ) {!   !} i)
    -- where
    --   +-η-helper : ∀ {Y} vc → (γ : pr₁ (∣ {!   !} ∣ Y)) → case+-sem A B (A +ₗ B) vc
    --      (λ x a₁ → inr (inl {!  ∣ a ∣  !}))
    --      (λ x b₁ → inr (inr {!   !})) ≡ vc 
    --   +-η-helper = {!   !}   
 
-- case+-sem A B (A +ₗ B) (∣ t ∣ γ)
--          (λ x a₁ → inr (inl (∣ a ∣ (Γ ! γ [ x ] , a₁))))
--          (λ x b₁ → inr (inr (∣ b ∣ (Γ ! γ [ x ] , b₁))))
-- i = i1 ⊢ ∣ t ∣ γ
...
Normalization : D.DepModel In {lsuc lzero} {lzero}
Normalization .D.DepModel.Con∙ = Con
Normalization .D.DepModel.Sub∙ = Sub
Normalization .D.DepModel.SubSet∙ = {!   !}
Normalization .D.DepModel._∘∙_ = _∘_
Normalization .D.DepModel.assoc∙ = assoc
Normalization .D.DepModel.id∙ = id
Normalization .D.DepModel.idr∙ = idr 
Normalization .D.DepModel.idl∙ = idl
Normalization .D.DepModel.Ty∙ = Ty
Normalization .D.DepModel.Tm∙ = Tm
Normalization .D.DepModel.TmSet∙ = {!   !}
Normalization .D.DepModel._[_]∙ = _[_]
Normalization .D.DepModel.[]-∘∙ = []-∘
Normalization .D.DepModel.[]-id∙ = []-id
Normalization .D.DepModel._▸∙_ = _▸_
Normalization .D.DepModel.p∙ = p
Normalization .D.DepModel.q∙ = q
Normalization .D.DepModel._,∙_ = _,_ₛ
Normalization .D.DepModel.,-∘∙ = ,-∘
Normalization .D.DepModel.▸-β₁∙ = ▸-β₁
Normalization .D.DepModel.▸-β₂∙ = ▸-β₂
Normalization .D.DepModel.▸-η∙ = ▸-η
Normalization .D.DepModel.◆∙ = ◆
Normalization .D.DepModel.ε∙ = ε
Normalization .D.DepModel.ε-∘∙ = ε-∘
Normalization .D.DepModel.◆-η∙ = ◆-η
Normalization .D.DepModel._⇒∙_ = _⇒_
Normalization .D.DepModel._+ₗ∙_ = _+ₗ_
Normalization .D.DepModel.app∙ = app
Normalization .D.DepModel.app-[]∙ = app-[]
Normalization .D.DepModel.lam∙ = lam
Normalization .D.DepModel.lam-[]∙ = lam-[]
Normalization .D.DepModel.⇒-β∙ = ⇒-β
Normalization .D.DepModel.⇒-η∙ = ⇒-η
Normalization .D.DepModel.⊥ₗ∙ = ⊥ₗ
Normalization .D.DepModel.exfalsoₗ∙ = exfalsoₗ
Normalization .D.DepModel.exfalsoₗ-[]∙ = exfalsoₗ-[]
Normalization .D.DepModel.Unitₗ∙ = Unitₗ
Normalization .D.DepModel.ttₗ∙ = ttₗ
Normalization .D.DepModel.ttₗ-[]∙ = ttₗ-[]
Normalization .D.DepModel.unit-η∙ = unit-η
Normalization .D.DepModel.inlₗ∙ = inlₗ
Normalization .D.DepModel.inlₗ-[]∙ = inlₗ-[]
Normalization .D.DepModel.inrₗ∙ = inrₗ
Normalization .D.DepModel.inrₗ-[]∙ = inrₗ-[]
Normalization .D.DepModel.case+∙ = case+
Normalization .D.DepModel.case+[]∙ = case+[]
Normalization .D.DepModel.+-β₁∙ = +-β₁
Normalization .D.DepModel.+-β₂∙ = +-β₂ 

open stlc-sum.Induction Normalization
...
-- -- map-ref-id : ∀ Γ → ⟦ Γ ⟧Cᵢ .map (ref-id Γ) ≡ S.id
-- -- map-ref-id = {!   !}


-- -- reflect : S.Tm Γ A → (⟦ A ⟧Tᵢ .∣_∣  {!   !})
-- -- reflect  = {!   !}
